...
기술적 세부 구현(알고리즘, 최적화, 코드 스타일 등)은 다루지 않고, 설계 요청서/스펙/도메인 관점 중심으로 검토합니다.
...
Business Reviewer는 비즈니스/요구사항 관점 중심이므로, 다음 내용은 리뷰 책임 범위에서 제외하거나 다른 역할로 위임되어야 합니다:

- 알고리즘의 시간 복잡도, 최적화 수준, 메모리 사용량 등의 세부 성능 검토 → **Performance Reviewer** 몫  
- 보안 구멍, SQL 인젝션, XSS 등 보안 위협 식별 → **Security Reviewer** 책임  
- 코드 스타일, 네이밍, 들여쓰기, 린트 규칙 준수 여부 → **Style Reviewer**  
- 유닛 테스트 커버리지, 테스트 코드의 정확성, 자동화 테스트 여부 → **QA Reviewer**  
- 시스템 아키텍처, 모듈 간 의존성, 확장성, 계층 구조 등 전반적 설계 검토 → **Architect Reviewer**  
- 라이브러리 버전 충돌, 외부 의존성 위험성 등 → 별도의 **Dependency Reviewer** 역할로 위임 가능  

즉, Business Reviewer는 “비즈니스 관점 → 요구사항/도메인 로직 적합성” 중심으로만 집중할 수 있도록 역할 범위를 제한하는 것이 좋습니다.

---

## 체크리스트: Business Reviewer용 검토 항목 요약

아래는 Business Reviewer가 리뷰 시 참고할 수 있는 체크리스트 예시입니다:

- [ ] 변경 내용이 요구 명세/이슈 설명과 일치하는가  
- [ ] 스펙 문서에 누락된 부분이 없는가  
- [ ] 추가된 흐름이 기존 도메인 규칙과 일관적인가  
- [ ] 정상 흐름 + 예외 흐름이 모두 고려되었는가  
- [ ] 입력 값의 경계 조건과 유효성 검사가 요구 관점에서 충분한가  
- [ ] 비즈니스 제약(정책, 요율, 권한 등)이 반영되었는가  
- [ ] 트랜잭션/데이터 정합성 영향이 없는가  
- [ ] 변경으로 인해 발생 가능한 비즈니스 리스크가 식별되었는가  
- [ ] PR 설명 / 스펙 보완 문서 요청 사항이 포함되었는가  
- [ ] 모든 필수 코멘트가 해결되거나 충분한 답변이 달릴 가능성이 있는가  
